...
DELETE FROM ART_WORK WHERE ART_ID = O.ARTIST_ID;
DELETE FROM CUSTOMER WHERE PURCHASED_ID = (SELECT ARTWORK_ID FROM ART_WORK WHERE ART_WORK.ART_ID = OLD.ARTIST_ID);
...
ADD CONSTRAINT MIN_PRICE CHECK (PRICE < 9999999999999);
...
ALTER TABLE ARTIST
ADD CONSTRAINT ID_CHECK CHECK (ARTIST_ID < 1000);

ALTER TABLE ARTIST ADD CONSTRAINT
BANGALI_CHECK CHECK (UPPER(BIRTHPLACE) NOT LIKE '%BANGLADESH%' AND UPPER(NAME) NOT IN ('SOHEL' , 'SAAD' , 'RONY' , 'DIPESH'));

INSERT INTO ARTIST (ARTIST_ID, NAME, BIRTHPLACE, STYLE_OF_ART) VALUES (18343903, 'SOHEL', 'Málaga, Spain', 'Cubism');

SELECT * FROM ARTIST; 


ALTER TABLE ARTIST
DROP CONSTRAINT BANGALI_CHECK;